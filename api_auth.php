...
// ================================================
// ✅ HANDLE OPTIONS PRE-FLIGHT REQUEST IMMEDIATELY
// ================================================
...
try {
    ...
    error_log('API auth config error: ' . $e->getMessage());
...
if (file_exists('system_logger.php')) {
    try {
        ...
        error_log('System logger load failed: ' . $e->getMessage());
    }
}
try {
    require_once 'token_auth.php';
} catch (Throwable $e) {
    error_log('Token auth load failed: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode([
        'status' => 'error',
        'message' => 'Authentication module failed to load'
    ]);
    exit;
}
...
// ...
// [Keep the rest of your code unchanged below this point]
// ...
// ============================================================================
