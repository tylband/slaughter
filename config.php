<?php
require_once dirname(__DIR__) . '/vendor/autoload.php'; // Autoload dependencies
use Dotenv\Dotenv;

// Load root .env file first for database configuration
$rootDir = dirname(__DIR__); // Get parent directory of api/ folder
$rootDotenv = Dotenv::createImmutable($rootDir);
$rootDotenv->load();
...
if (strpos($scriptDir, $rootDir . '/system') === 0 || strpos($callingScript, 'system/') !== false) {
    // Load .env from system folder for system PHP files (to override root settings if needed)
    $dotenv = Dotenv::createImmutable($rootDir . '/system');
    $dotenv->load();
