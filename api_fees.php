...
try {
    ...
    error_log('API Session Debug - Username: ' . ($_SESSION['username'] ?? 'NOT SET') . ', UserID: ' . ($_SESSION['user_id'] ?? 'NOT SET'));
...
if ($_SERVER['REQUEST_METHOD'] !== 'OPTIONS') {
    if (!isset($_SESSION['username']) || !isset($_SESSION['user_id'])) {
        // For testing purposes, create a temporary session if test parameter is provided
        if (isset($_GET['test']) && $_GET['test'] === '1') {
            $_SESSION['username'] = 'test_user';
            $_SESSION['user_id'] = 1;
            error_log('Created test session for API debugging');
        } else {
    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            try {
                ...
                $logger = new SystemLogger($conn, $_SESSION['user_id'], $_SESSION['username']);
